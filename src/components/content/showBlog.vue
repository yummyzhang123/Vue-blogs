<template>
  <el-container direction="vertical"  v-theme:bg="'wide'">
    <el-row type="flex" justify="center" >
      <el-col >
        <div class="blog-box" v-for="blog in filteredBlogs" v-box="'box'" :key="blog.id">
...
<script>
  import axios from 'axios'
    export default {
      ...
      props:['search','tag'],//标识查找条件是用户名还是博客内容
      data(){
          return {
            blog:[],
          }
      ...
      created(){     //读取服务器数据
        ...
        /*this.$http.get('https://vue-blog-7e4bf-default-rtdb.firebaseio.com/')
          .then((data)=>{
        ...
        /*axios请求http的方法*/
        this.$axios.get('/api/blogs/blogs')
          .then((data)=>{
            return data.data.data;       //axios方法请求的是一个data对象（所有内容都存储在data对象里）
      ...
      computed:{
        filteredBlogs(){       //过滤搜索结果
          return this.blog.filter((blog)=>{
            ...
            if(this.tag==1){
              if(blog.author==this.search){
                return blog.author==this.search;
              }
            }
            else{             
