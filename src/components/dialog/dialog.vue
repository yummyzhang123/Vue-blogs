  ...
  import axios from 'axios'
    export default {
      ...
      methods:{
        /*把blog内容存储到服务器*/
        confirmed(){
          // this.$http.post("https://vue-blog-7e4bf-default-rtdb.firebaseio.com/posts.json", this.blog)    //vue-resource请求方法
          if (this.dialogEvent == '发布') {
            this.$axios.post("/api/blogs/add", this.blog)    //axios请求方法
              .then((res)=>{
                if(res.data.code===0){
                  this.$message({
                    message:'发布成功',
            ...
            this.$axios.delete('/api/blogs/delete')     //axios请求方法
              .then(response=>{
